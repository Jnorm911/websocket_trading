...
def fit_and_evaluate(X, y, selected_features, n_splits=5):
    ...
    for method, features in selected_features.items():
        ...
        if not X_selected.empty:
            # Parallelize the model fitting and evaluation
            model_results = Parallel(n_jobs=-1)(
                delayed(fit_and_evaluate_one_model)(
                    model_name, model, method, features, X, y, n_splits
                )
            ...
            for result in model_results:
                if result is not None:
                    key = f"{result['method']}_{result['model_name']}"
                    results[key] = {
                        "mean_score": result["mean_score"],
                        "best_features": result["best_features"],
                    }
                    print(
                        f"Mean cross_val_score for {result['model_name']} with features selected by {result['method']}: {result['mean_score']}"
                    )
        else:
            print(f"No features selected by {method}. Skipping model fitting.")
