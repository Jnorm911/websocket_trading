...
def evaluate_fold(train_index, test_index, clf, metrics):
    ...
    y_pred_proba = clf.predict_proba(X_test_fold)[
        :, 1
    ]  # Predict probabilities for roc_auc_score

    # Calculate the evaluation metrics
    fold_scores = {}
    for metric, score_func in metrics.items():
        if metric == "roc_auc":
            score = score_func(y_test_fold, y_pred_proba)
        else:
...
degree = 2  # Set the desired polynomial degree
...
n_components = 10  # Set the desired number of principal components
pca = PCA(n_components)
X_pca = pca.fit_transform(X_poly_standardized)

# Now you can use X_pca and y_train with your classifiers
...
for name, clf in tqdm(classifiers, desc="Classifiers"):
    ...
    fold_scores_list = Parallel(n_jobs=-1)(
        ...
        for train_index, test_index in kf.split(X_pca, y_train)
    )
